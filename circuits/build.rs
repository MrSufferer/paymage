...
const GROTH16_KEY_CIRCUITS: &[&str] = &["policy_tx_2_2", "selectiveDisclosure_1", "compliantIntent_1", "payroll_20", "payroll_10_10", "payrollWithdraw_10"];
...
fn circuit_needs_groth16_keys(name: &str) -> bool {
    if let Ok(only) = env::var("ONLY_KEY_CIRCUITS") {
        return only
            .split(',')
            .map(str::trim)
            .filter(|s| !s.is_empty())
