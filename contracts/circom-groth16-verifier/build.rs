...
fn main() {
    ...
    // .cargo/config.toml sets VERIFIER_VK_JSON for local builds.
    // On CI fast paths (no circom), the file may not exist.
    ...
    if let Some(path) = path {
        ...
        // VK files absent (CI fast path skips Circom). Write zero-filled
        // placeholders so the crate still compiles for `cargo check` and
        // unit tests that call `verify_with_vk` directly.
        println!("cargo:warning=circom-groth16-verifier: VK file absent; using zero placeholders");
        let placeholder =
            "// Auto-generated by build.rs — zero-filled placeholders (no VK available).\n"
                .to_string()
                ...
                + "const VK_IC: [[u8; 64]; 1] = [[0u8; 64]; 1];\n";
        fs::write(out_dir.join("vk.rs"), placeholder).expect("failed to write vk.rs");
    }
}
