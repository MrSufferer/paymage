...
## Browser Payroll Testnet Unblock Pass (2026-07-14)

- Restored the dashboard local verification baseline by installing dashboard dependencies, regenerating the Poseidon WASM binding, adding the missing development `StellarDebugPanel`, and adding an explicit payroll prover shim that fails loudly until the real `payroll_prover_bg.wasm` binding is generated.
- Added `zk-payroll-dashboard/lib/zk/payrollInputs.ts` to keep proof-critical browser input preparation out of React components:
  - maps application employee IDs such as `emp_001` to deterministic BN254 field-element decimal strings;
  - generates cryptographically random field-element salts;
  - preserves the generated salts when building the real `zkEngine.generateProof()` private input lists;
  - derives `commitmentId = Poseidon2(commitment, domain=0x02)`, matching `PayrollWithdrawCircuit`.
- Extended `app/crates/poseidon-wasm` with `poseidon2_commitment_id()` and regenerated `zk-payroll-dashboard/lib/zk/wasm/poseidon/*` so browser commitment IDs match the withdraw circuit.
- Wired both `SetEmployeeRoot` and `PayrollWizard` through the shared payroll slot builder. This fixes the previous root/proof mismatch where the wizard built a tree with random salts but passed `salt: "0"` to the real prover.
- Updated `PayrollWizard` IPFS CID preparation to store contract commitment records by the derived commitment ID instead of using the raw commitment as the key.
- Added `NEXT_PUBLIC_ZK_ARTIFACTS_URL` to `.env.example` and updated middleware CSP to allow the configured artifact origin in `connect-src`, so the real prover can fetch large PK/R1CS artifacts from a local artifact server or CDN.

Known remaining blocker: the real payroll prover WASM binding was not generated in this checkout. `wasm-pack build --release --target web --out-dir ../../../zk-payroll-dashboard/lib/zk/wasm app/crates/payroll-prover` started compiling after network access was granted, but was interrupted after several silent minutes. Until `payroll_prover_bg.wasm` and the large circuit artifacts are restored, `NEXT_PUBLIC_ZK_ENGINE=real` will initialize artifact fetches but fail loudly if the shimmed prover is invoked.

## Testnet dApp + Vercel Preview Pass (2026-07-15)

- Added shared Soroban transaction confirmation helpers in `zk-payroll-dashboard/lib/stellar/transactions.ts`.
- Wired confirmation polling into the shared `StellarProvider.invokeContract()` path, the employee-root posting flow, and the compliance/auditor contract-call flow. These flows now wait for `getTransaction(hash) -> SUCCESS` and surface failed/non-confirmed transactions instead of treating `sendTransaction()` acceptance as final success.
- Added a real-proof server mode for the dashboard:
  - `NEXT_PUBLIC_ZK_ENGINE=server` selects a browser engine that builds the exact payroll circuit input, then calls `/api/zk/payroll/prove`.
  - `/api/zk/payroll/prove` validates the public statement (`employeeRoot`, `totalPayrollAmount`, `payrollPeriodId`) before proxying to a server-only `PAYROLL_PROVER_URL`.
  - `e2e-tests/src/bin/payroll_prover_service.rs` is a small native prover service/stdio adapter using the same `prover` + `witness` crates as the native E2E path.
  - `zk-payroll-dashboard/lib/zk/payrollCircuitInput.ts` normalizes browser Poseidon hex roots/path elements into decimal Circom inputs, matching the native E2E input shape.
  - `app/crates/payroll-prover` now returns public inputs as big-endian 32-byte field hex so Soroban `U256` encoding matches the contract verifier.
- Deployed a Vercel preview with the current testnet contract IDs and `NEXT_PUBLIC_ZK_ENGINE=mock` because real browser proving artifacts are not present in this checkout:
  - Preview: https://staging-4gl0cp6e1-gadillacers-projects.vercel.app
  - Inspect: https://vercel.com/gadillacers-projects/staging/EQgFLHE8FUDuR6CoBP3rdE1u7CpY
- Fresh native testnet E2E command was started but produced no output for several minutes after compilation and was interrupted. Historical native testnet proof transactions remain documented in the testing doc, but this session did not produce fresh testnet transaction hashes.
- Follow-up in this session:
  - targeted `BUILD_TESTS=1 REGEN_KEYS=1 ONLY_KEY_CIRCUITS=payroll_10_10,payrollWithdraw_10 cargo build -p circuits` completed and regenerated local `payroll_10_10`/`payrollWithdraw_10` proving keys plus debug R1CS/WASM artifacts;
  - scoped `BUILD_TESTS=1 ONLY_KEY_CIRCUITS=payroll_10_10,payrollWithdraw_10 cargo build -p e2e-tests --bin payroll_prover_service` completed;
  - a real browser-shaped proof request generated by dashboard helpers was fed through `payroll_prover_service --once`, which emitted a 256-byte proof and 3 public inputs for root `3047185273bc91e8f524602b45d7f1b4544166272254f58007de24cd664b9552`, amount `5000000`, period `1`;
  - a detached local prover service and Serveo HTTPS tunnel were started via `screen`;
  - a Vercel preview was deployed with `NEXT_PUBLIC_ZK_ENGINE=server` and `PAYROLL_PROVER_URL` pointing at the detached tunnel:
...
Operational caveat: the Serveo tunnel is a temporary preview dependency, not production infrastructure. If the local `screen` sessions stop or the machine sleeps, proof generation from the Vercel preview will fail until the prover is restarted and Vercel is redeployed or reconfigured with a new `PAYROLL_PROVER_URL`.

## Playwright Dashboard Proof Automation (2026-07-15)

- Added Playwright test infrastructure to `zk-payroll-dashboard`:
  - `npm run test:e2e` runs `playwright test`;
  - `playwright.config.ts` starts a local Next dev server by default with testnet contract IDs and `NEXT_PUBLIC_ZK_ENGINE=server`;
  - when `PLAYWRIGHT_BASE_URL` or `PLAYWRIGHT_SKIP_WEBSERVER=1` is set, the same tests target an already deployed site;
  - when `VERCEL_AUTOMATION_BYPASS_SECRET` is set, Playwright sends `x-vercel-protection-bypass` and `x-vercel-set-bypass-cookie: true` so protected Vercel previews can be tested automatically.
- Added `e2e/payroll-server-proof.spec.ts`, which mocks the Freighter browser bridge, signs the app's real challenge transaction with `E2E_STELLAR_SECRET_KEY`, seeds dashboard state with 10 active employees, drives `/payroll/execute`, and asserts the server-proof API returns a 256-byte Groth16 proof plus three 32-byte public inputs.
- Fixed the development `StellarDebugPanel` to use `pointer-events-none`; it was visually useful but intercepted Playwright clicks on the wizard in smaller Chromium viewports.
- Root-cause for failed manual Freighter login on the Vercel preview: Vercel Deployment Protection is enabled. Fresh `curl` checks against `/login`, `/api/health`, and `/api/auth/challenge` all returned `302` redirects to `https://vercel.com/sso-api...`, so requests never reached the dashboard's wallet challenge route. Disable preview protection, use a Vercel share/bypass link for manual testing, or configure Protection Bypass for Automation for Playwright.
...
Result: 1/1 Playwright test passed in 42.8s against the local Next dev server and native prover service on `127.0.0.1:8788`.
