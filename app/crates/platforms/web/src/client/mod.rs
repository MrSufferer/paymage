...
impl WebClient {
    ...
    #[wasm_bindgen(js_name = generatePayrollProof)]
    #[allow(clippy::too_many_arguments)]
    pub async fn generate_payroll_proof(
        &self,
        employee_root_hex: String,
        total_payroll_amount_hex: String,
        payroll_period_id_hex: String,
        actual_employee_count: u32,
        employee_ids_hex: Array,
        salary_amounts_hex: Array,
        salts_hex: Array,
        path_elements: Array,
        path_indices: Array,
    ) -> Result<JsValue, JsError> {
        ...
        let employee_root = parse_field_hex_str(&employee_root_hex)?;
        let total_payroll_amount = parse_field_hex_str(&total_payroll_amount_hex)?;
        let payroll_period_id = parse_field_hex_str(&payroll_period_id_hex)?;

        let employee_ids = hex_array_to_fields(&employee_ids_hex)?;
        let salary_amounts = hex_array_to_fields(&salary_amounts_hex)?;
        let salts = hex_array_to_fields(&salts_hex)?;

        let path_elements = parse_2d_hex_array(&path_elements)?;
        let path_indices = parse_u32_array(&path_indices)?;

        let req = PayrollProverRequest {
            employee_root,
        ...
        let resp = match self
            .prover_request(ProverWorkerRequest::Payroll(req), 120_000)
            .await?
        {
            ProverWorkerResponse::PayrollPrepared(r) => r,
            other => {
                return Err(JsError::new(&format!(
                    "unexpected prover response: {:?}",
                    other
                )));
            }
        };
...
fn hex_array_to_fields(arr: &Array) -> Result<Vec<Field>, JsError> {
    let len = arr.length();
    ...
    for i in 0..len {
        ...
        let s = v
            .as_string()
            .ok_or_else(|| JsError::new("hex array element must be a string"))?;
        out.push(parse_field_hex_str(&s)?);
    }
    Ok(out)
}

fn parse_2d_hex_array(arr: &Array) -> Result<Vec<Vec<Field>>, JsError> {
    let len = arr.length();
    let mut out = Vec::with_capacity(len as usize);
    for i in 0..len {
        let v = arr.get(i);
        let inner: Array = v
            .dyn_into()
            .map_err(|_| JsError::new("path_elements must be an array of arrays"))?;
        out.push(hex_array_to_fields(&inner)?);
    }
    Ok(out)
}
...
fn parse_u32_array(arr: &Array) -> Result<Vec<u32>, JsError> {
    ...
    for i in 0..len {
        ...
        let n: f64 = v
            .as_f64()
            .ok_or_else(|| JsError::new("path_indices element must be a number"))?;
        out.push(n as u32);
    }
    Ok(out)
}
