...
thread_local! {
    ...
    static PAYROLL_WITNESS_CALC: RefCell<Option<WitnessCalculator>> = const { RefCell::new(None) };
    static PAYROLL_PROVER: RefCell<Option<Prover>> = const { RefCell::new(None) };
}
...
async fn load_circuit_artifacts() -> Result<(), JsError> {
    ...
    let (wasm_bytes, r1cs_bytes, disc_wasm_bytes, disc_r1cs_bytes, payroll_wasm_bytes, payroll_r1cs_bytes) = try_join!(
        ...
        async {
            let wasm_bytes: Vec<u8> = fetch_circuit_file("circuits/payroll_20.wasm").await?;
            log::debug!(
                "[{WORKER_NAME}] fetched payroll_20.wasm: {} bytes",
                wasm_bytes.len()
            );
            Ok::<Vec<u8>, JsError>(wasm_bytes)
        },
        async {
            let r1cs_bytes: Vec<u8> = fetch_circuit_file("circuits/payroll_20.r1cs").await?;
            log::debug!(
                "[{WORKER_NAME}] fetched payroll_20.r1cs: {} bytes",
                r1cs_bytes.len()
            );
            Ok::<Vec<u8>, JsError>(r1cs_bytes)
        }
    ...
    ensure_sha256_matches(
        "payroll_20_proving_key.bin",
        PAYROLL_PROVING_KEY,
        crate::artifact_hashes::EXPECTED_PAYROLL_20_PROVING_KEY_LEN,
        crate::artifact_hashes::EXPECTED_PAYROLL_20_PROVING_KEY_SHA256,
    )?;
    ensure_sha256_matches(
        "payroll_20.wasm",
        &payroll_wasm_bytes,
        crate::artifact_hashes::EXPECTED_PAYROLL_20_WASM_LEN,
        crate::artifact_hashes::EXPECTED_PAYROLL_20_WASM_SHA256,
    )?;
    ensure_sha256_matches(
        "payroll_20.r1cs",
        &payroll_r1cs_bytes,
        crate::artifact_hashes::EXPECTED_PAYROLL_20_R1CS_LEN,
        crate::artifact_hashes::EXPECTED_PAYROLL_20_R1CS_SHA256,
    )?;
    ...
    PAYROLL_WITNESS_CALC.with(|cell| {
        *cell.borrow_mut() = Some(payroll_witness_calc);
    });
    PAYROLL_PROVER.with(|cell| {
        *cell.borrow_mut() = Some(payroll_prover);
    });
    Ok(())
}
...
pub(crate) async fn router(req: ProverWorkerRequest) -> Result<ProverWorkerResponse> {
    let resp = match req {
        ...
        ProverWorkerRequest::Payroll(req) => {
            log::debug!("[{WORKER_NAME}] payroll");

            let params = prover::flows::PayrollParams {
                employee_root: req.employee_root,
                total_payroll_amount: req.total_payroll_amount,
                payroll_period_id: req.payroll_period_id,
                actual_employee_count: req.actual_employee_count,
                employee_ids: req.employee_ids,
                salary_amounts: req.salary_amounts,
                salts: req.salts,
                path_elements: req.path_elements,
                path_indices: req.path_indices,
            };
            ...
            let witness_bytes = PAYROLL_WITNESS_CALC.with(|cell| {
                ...
                let calc = borrow
                    .as_mut()
                    .ok_or_else(|| anyhow::anyhow!("payroll witness calculator not initialized"))?;
                calc.compute_witness(&circuit_inputs_json)
                    .context("payroll witness calculation failed")
            })?;

            let (proof_uncompressed, public_inputs) = PAYROLL_PROVER.with(|cell| {
                let borrow = cell.borrow();
                let prover = borrow
                    .as_ref()
                    .ok_or_else(|| anyhow::anyhow!("payroll prover not initialized"))?;

                log::debug!("[{WORKER_NAME}] prove payroll");
                let proof_compressed = prover.prove_bytes(&witness_bytes)?;
                let public_inputs_vec = prover.extract_public_inputs(&witness_bytes)?;
                log::debug!("[{WORKER_NAME}] verify payroll proof");
                let ok = prover.verify(&proof_compressed, &public_inputs_vec)?;
                if !ok {
                    return Err(anyhow::anyhow!("payroll proof verification failed"));
                }

                let proof_uncompressed = prover.proof_bytes_to_uncompressed(&proof_compressed)?;
                if proof_uncompressed.len() != 256 {
                    return Err(anyhow::anyhow!(
                        "unexpected payroll proof length: {}",
