...
pub enum ProverWorkerResponse {
    ...
    PayrollPrepared(PayrollProverResponse),
}
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PayrollProverRequest {
    /// Public input: Merkle root of the employee commitment tree.
    pub employee_root: Field,
    /// Public input: Sum of all salaries in the batch.
    pub total_payroll_amount: Field,
    /// Public input: Monotonic period counter (prevents replay).
    pub payroll_period_id: Field,
    /// Number of employees in this batch.
    pub actual_employee_count: u32,
    /// Employee IDs (BN254 field elements as hex strings).
    ...
    /// Merkle proof path indices per employee.
    pub path_indices: Vec<u32>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PayrollProverResponse {
    /// Public inputs used: [employeeRoot, totalPayrollAmount, payrollPeriodId].
    pub public_inputs: [Field; 3],
    /// Uncompressed 256-byte Groth16 proof: A(64) || B(128) || C(64).
    pub proof_uncompressed: Vec<u8>,
}
