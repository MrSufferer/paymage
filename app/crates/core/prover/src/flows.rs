...
use types::{
    ...
    NoteAmount, NotePrivateKey, NotePublicKey, U256,
};
...
/// Parameters for the payroll batch proof circuit.
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PayrollParams {
    /// Merkle root of the employee commitment tree (public input).
    pub employee_root: Field,
    /// Sum of all salaries in the batch in stroops (public input).
    pub total_payroll_amount: Field,
    /// Monotonic period counter to prevent replay (public input).
    pub payroll_period_id: Field,
    /// Number of actual employees in this batch.
    pub actual_employee_count: u32,
    /// Employee IDs as field elements (hex strings).
    pub employee_ids: Vec<Field>,
    /// Salary amounts in stroops as field elements (hex strings).
    pub salary_amounts: Vec<Field>,
    /// Random salts as field elements (hex strings).
    pub salts: Vec<Field>,
    /// Merkle proof path elements per employee.
    pub path_elements: Vec<Vec<Field>>,
    /// Merkle proof path indices per employee (as u32 bitmasks).
    pub path_indices: Vec<u32>,
}

/// Artifacts generated for payroll proof.
#[derive(Clone, Debug)]
pub struct PayrollArtifacts {
    /// Circuit inputs for the witness calculator.
    pub circuit_inputs: CircuitInputs,
    /// Public inputs used: [employeeRoot, totalPayrollAmount, payrollPeriodId].
    pub public_inputs: [Field; 3],
}

/// Generates circuit inputs for the PayrollBatch circuit.
///
/// # Payroll circuit signal names
/// - Public: `employeeRoot`, `totalPayrollAmount`, `payrollPeriodId`
/// - Private: `employeeId[n]`, `salaryAmount[n]`, `salt[n]`,
///   `pathElements[n][levels]`, `pathIndices[n]`
///
/// The circuit iterates over all `n` slots (max 500). For unused slots,
/// zero salary amounts naturally satisfy the sum constraint.
pub fn payroll_proof(params: PayrollParams) -> Result<PayrollArtifacts> {
    let PayrollParams {
        employee_root,
    ...
    let n = employee_ids.len();
    if n != salary_amounts.len()
        || n != salts.len()
        || n != path_elements.len()
        || n != path_indices.len()
    {
        return Err(anyhow!(
            "payroll params length mismatch: ids={}, salaries={}, salts={}, path_elements={}, path_indices={}",
            employee_ids.len(),
            salary_amounts.len(),
            salts.len(),
            path_elements.len(),
    ...
    for (i, pe) in path_elements.iter().enumerate() {
        if pe.len() != levels {
            return Err(anyhow!(
                "path_elements[{}] has {} elements, expected {}",
                i,
                pe.len(),
                levels
            ));
    ...
    circuit.set_single(
        "totalPayrollAmount",
        &field_to_circuit_hex(&total_payroll_amount)?,
    );
    circuit.set_single(
        "payrollPeriodId",
    ...
    let mut salts_hex = Vec::with_capacity(n);
    let mut path_indices_hex = Vec::with_capacity(n);
    let mut path_elements_flat_hex: Vec<String> = Vec::new();

    for i in 0..n {
        employee_ids_hex.push(field_to_circuit_hex(&employee_ids[i])?);
        salary_amounts_hex.push(field_to_circuit_hex(&salary_amounts[i])?);
        salts_hex.push(field_to_circuit_hex(&salts[i])?);

        // pathIndices is a u32 bitmask — the circom Num2Bits interprets the
        // field element's byte representation as little-endian bits.
        // We store the u32 as a single field element.
    ...
    circuit.set_array("employeeId", employee_ids_hex);
    circuit.set_array("salaryAmount", salary_amounts_hex);
    circuit.set_array("salt", salts_hex);
    circuit.set_array("pathIndices", path_indices_hex);
    circuit.set_array("pathElements", path_elements_flat_hex);

    Ok(PayrollArtifacts {
        circuit_inputs: circuit,
        public_inputs: [employee_root, total_payroll_amount, payroll_period_id],
    })
}
