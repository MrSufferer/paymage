...
This hash is `disclosure::vk_hash_hex` over the **compressed arkworks verifying-key bytes** (`VerifyingKey::serialize_compressed`), not the SHA-256 of the JSON file. The same value is pinned in `app/js/disclosure.js`.
...
- `policy_tx_2_2`: legacy policy transaction ceremony
