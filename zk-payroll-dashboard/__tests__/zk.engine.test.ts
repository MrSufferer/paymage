...
describe("zkEngine init", () => {
  ...
  it("falls back cleanly when artifact fetches are unavailable", async () => {
    ...
    expect(proof.publicSignals.length).toBe(3);
    ...
    expect(
      fetchCalls.some(
        (call) => call.input === "/zk/payroll_10_10.wasm" && call.init?.cache === "force-cache"
      )
    ).toBe(true);
  });
  ...
  it("caches singleton initialization across concurrent calls", async () => {
    ...
    expect(
      fetchCalls.filter((call) => call.input === "/zk/payroll_10_10.wasm").length
    ).toBe(1);
  });
});
